...
//   const CoefficientVector<data_t>& coeffs;
//   data_t& beta;
//   int& nmax;
//   data_t& f;
//   data_t& chi2;
...
//   reg_parameters * p = (reg_parameters *)params;
//   NumVector<data_t> coeffs = p->coeffs;

//     //std::cout << coeffs << std::endl;

//   // copy coeffs from gsl vector into new coeff vector
//   for (int i=0; i< v->size; i++)
//     coeffs(i) = gsl_vector_get(v, i);
  
//   // update model with these coeffs
//   p->d.setCoeffs(coeffs);
//   p->d.fixCoeffs(true);
...
//   data_t posFlux, negFlux;
//   computeFluxes(p->reco,negFlux,posFlux,p->R,p->H);
//   p->f = p->chi2 + (p->lambda)*p->H;
...
//   min = gsl_multimin_fminimizer_x(s);
//   f_beta(min,&p);
//   p.history << "# " << iter << ":\t beta = " << gsl_vector_get (min, 0)<< "\t f = " << gsl_multimin_fminimizer_minimum(s) << "\t chi^2 = " << p.chi2 <<  "\t R = "<< p.R << std::endl;
...
//   const CoefficientVector<data_t>& coeffVector = p.coeffs;
//   std::cout << p.coeffs << std::endl;
...
//   gsl_vector *min;
//   min = gsl_multimin_fminimizer_x(s);
//   data_t f = f_coeffs(min,&p);

//   //p.history << "# " << iter << ":\t f = " << gsl_multimin_fminimizer_minimum(s) << "\t chi^2 = " << p.chi2 << "\t R = " << p.R << std::endl;
//   p.history << "# " << iter << ":\t f = " << f << "\t chi^2 = " << p.chi2 << "\t R = " << p.R << std::endl;
...
// void OptimalDecomposite2D::appendRegResults(std::vector<regResults>& results, int nmax, data_t f, data_t lambda, data_t beta, data_t chi2, data_t R, const CoefficientVector<data_t>& coeffs) {
...
//   const NumVector<data_t>& model = Decomposite2D::C2D.getModel();
//   const NumVector<data_t>& residuals = Decomposite2D::getResiduals();
//   const NumVector<data_t>& coeffs = Decomposite2D::C2D.getCoeffs();
...
//       (Decomposite2D::C2D).setCoeffs(bestResult.coeffs);
