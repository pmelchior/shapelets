...
SExFrameExtended::SExFrameExtended (std::string fitsfile) : FitsImage<double>(fitsfile) {
  text << "# Reading FITS file " << fitsfile << endl;
  text << "# Image properties: size = "<< FitsImage<double>::getSize(0) << "/" << FitsImage<double>::getSize(1) << std::endl; 
  history.append(text);
  SExCatFormatExtended empty = {0,0,0,0,0,0,0,0};
  sf = empty;
...
SExFrameExtended::~SExFrameExtended() {
  gsl_rng_free (r);
}

void SExFrameExtended::readCatalog(std::string catfile) {
  // first inser empty object 0, since SExtractor starts with NUMBER 1
  objectList.clear();
  SExCatObjectExtended s0 = {0,0,0,0,0,0,0,0};
  objectList.push_back(s0);
  // open cat file
  ifstream catalog (catfile.c_str());
  if (catalog.fail()) {
    history.append("SExFrameExtended: catalog file does not exist!\n");
    terminate();
  }
  catalog.clear();
  // read in cat file
...
void SExFrameExtended::readSegmentationMap(std::string segmentfile) {
  FitsImage<int>* seg = new FitsImage<int>(segmentfile);
  const NumVector<int>& fitsdata = seg->getData();
  for (int i=0; i< fitsdata.size(); i++)
    segMap(i) = (double) fitsdata(i);
  delete seg;
  segmapRead = 1;
}

unsigned int SExFrameExtended::getNumberOfObjects() {
  if (catRead)
...
void SExFrameExtended::setCharacteristicSize(uint size) {
  correlationLength = size;
}

void SExFrameExtended::fillObject(Object& O) {
...
void SExFrameExtended::findHalo(uint nr, int& xmin, int& xmax, int& ymin, int& ymax) {
  const NumVector<double>& data = FitsImage<double>::getData();
  ...
  for (int i=0; i < groups.size(); i++) {
    for(iter = groups[i].begin(); iter != groups[i].end(); iter++) {
      int pixel = *iter;
      int x, y;
      FitsImage<double>::getCoords(pixel,x,y);
...
void SExFrameExtended::subtractBackground() {
  subtractBG = 1;
}

// fill in the column position into the SExCatFormat struct
void SExFrameExtended::insertFormatField(std::string type, std::string columnnr) {
...
void SExFrameExtended::checkFormat() {
  bool trouble = 0;
  if (sf.NUMBER == 0) {
    text << "SExFrameExtended: NUMBER keyword not provided!" << std::endl;
    trouble = 1;
  }
  if (sf.XMIN_IMAGE == 0) {
    text << "SExFrameExtended: XMIN_IMAGE keyword not provided!" << std::endl;
    trouble = 1;
  }
  if (sf.XMAX_IMAGE == 0) {
    text << "SExFrameExtended: XMAX_IMAGE keyword not provided!" << std::endl;
    trouble = 1;
  }
  if (sf.YMIN_IMAGE == 0) {
    text << "SExFrameExtended: YMIN_IMAGE keyword not provided!" << std::endl;
    trouble = 1;
  }
  if (sf.YMAX_IMAGE == 0) {
    text << "SExFrameExtended: YMAX_IMAGE keyword not provided!" << std::endl;
    trouble = 1;
  }
  if (sf.BACKGROUND == 0) {
    text << "SExFrameExtended: BACKGROUND keyword not provided!" << std::endl;
    trouble = 1;
  }
  if (sf.FLAGS == 0) {
    text << "SExFrameExtended: FLAGS keyword not provided!" << std::endl;
    trouble = 1;
  }
  if (sf.CLASS_STAR == 0) {
    text << "SExFrameExtended: CLASS_STAR keyword not provided!" << std::endl;
    trouble = 1;
  }
  history.append(text);
...
void SExFrameExtended::addFrameBorder(double factor, int& xmin, int& xmax, int& ymin, int& ymax) { 
...
double SExFrameExtended::computeBlendingProbability(unsigned int nr) {
  ...
  if ((flag >> 1)%2 == 1) {
    ...
    for (int y = GSL_MAX_INT(ymin,0); y <= GSL_MIN_INT(ymax,axsize1-1); y++) {
      for (int x = GSL_MAX_INT(xmin,0); x <= GSL_MIN_INT(xmax,axsize0-1); x++) {
	unsigned int pixel = FitsImage<double>::getPixel(x,y);
...
int SExFrameExtended::neighborpixel(int pixel,unsigned int direction, int axsize0, int axsize1) {
  int x,y,index;
  FitsImage<double>::getCoords(pixel,x,y);
  switch(direction) {
  case 0: 
...
void SExFrameExtended::findObjectList(std::list<uint>& pixellist, int object, int xmin, int xmax, int ymin, int ymax) {
  pixellist.clear();
  for (int y = GSL_MAX_INT(ymin,0); y <= GSL_MIN_INT(ymax,axsize1-1); y++) {
    for (int x = GSL_MAX_INT(xmin,0); x <= GSL_MIN_INT(xmax,axsize0-1); x++) {
      uint j = FitsImage<double>::getPixel(x,y);
...
void SExFrameExtended::removeObjectInnerPixels(std::list<uint>&  pixellist, uint nr) {
...
void SExFrameExtended::refinePixelMap(std::list<uint>& pixellist, uint startpixel, bool positive, int xmin, int xmax, int ymin, int ymax, double bg_mean, double bg_rms) {
  const NumVector<double>& data = FitsImage<double>::getData();
  pixellist.clear();
...
void SExFrameExtended::makeLocalNoiseMap(uint nr, int outerxmin, int outerxmax, int outerymin, int outerymax, int innerxmin, int innerxmax, int innerymin, int innerymax, NumVector<double>& mean, NumVector<double>& rms) {
  ...
  for (int y = outerymin; y <= outerymax; y++) {
	...
	if (x>=0 && x < axsize0 && y >= 0 && y < axsize1)
	  getSubAreaStats(nr,FitsImage<double>::getPixel(x,y),correlationLength,mean(cell),rms(cell));
	else {
	  mean(cell) = bg_mean;
	  rms(cell) = bg_rms;
	}
...
void SExFrameExtended::getSubAreaStats(uint nr, uint startpixel, int sidelength, double& mean, double& rms) {
  const NumVector<double>& data = FitsImage<double>::getData();
  int npixels = (sidelength+1)*(sidelength+1);
  double subarea[npixels];
  int index = 0;
  int x0,y0;
  FitsImage<double>::getCoords(startpixel,x0,y0);
  uint pixel;
  for (int j=-sidelength/2; j<=sidelength/2; j++) {
    for (int i=-sidelength/2; i<=sidelength/2; i++) {
      int x = x0 + i, y = y0 + j;
      pixel = FitsImage<double>::getPixel(x,y);
      // are we inside the image region?
      if (x>=0 && x < axsize0 && y >= 0 && y < axsize1) {
...
void SExFrameExtended::paintSegmentBorder(int xmin, int xmax, int ymin, int ymax) {
...
void SExFrameExtended::cleanSegMapArea(int nr, int xmin, int xmax, int ymin, int ymax) {
  int runningnr;
  for (int y = GSL_MAX_INT(ymin,0); y <= GSL_MIN_INT(ymax,axsize1-1); y++) {
    for (int x = GSL_MAX_INT(xmin,0); x <= GSL_MIN_INT(xmax,axsize0-1); x++) {
      runningnr = nr;
...
const NumVector<double>& SExFrameExtended::getObjectMap() {
  return segMap;
}


// estimate noise by iterative sigma clipping
void SExFrameExtended::estimateNoise() {
...
double SExFrameExtended::getNoiseMean() {
  if (!estimatedBG) estimateNoise();
  return bg_mean;
}

double SExFrameExtended::getNoiseRMS() {
  if (!estimatedBG) estimateNoise();
  return bg_rms;
}

void SExFrameExtended::setNoiseMeanRMS(double mean, double rms) {
  estimatedBG = 1;
  bg_mean = mean;
  bg_rms = rms;
...
double SExFrameExtended::distanceFromRim(std::list<uint>& objectlist, uint pixel) {
  int x0, y0;
  FitsImage<double>::getCoords(pixel,x0,y0);
  int x,y;
  double mindist=axsize0;
  double distance;
  for(std::list<uint>::iterator iter = objectlist.begin(); iter != objectlist.end(); iter++) {
    FitsImage<double>::getCoords(*iter,x,y);
...
const History& SExFrameExtended::getHistory () {
  return history;
}
