...
class Grid {
 public:
  ...
  Point2D<data_t> operator() (unsigned long index) const;
  ...
  /// construction time).
  Point2D<int> getCoords(unsigned long pixel) const;
  /// Get the pixel index from the \p P with image coordinates.
  unsigned long getPixel(const Point2D<int>& P) const;
  ...
  long getNeighborPixel(const Point2D<int>& P, unsigned char direction) const;
