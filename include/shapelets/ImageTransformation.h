...
class ImageTransformation {
  ...
  void rotate(NumMatrix<Complex>& polarCoeffs, data_t rho, std::ostringstream& history);
  /// Apply convergence of \f$\kappa\f$ to the image.
  void converge(NumMatrix<Complex>& polarCoeffs, data_t& beta, data_t kappa, std::ostringstream& history);
  ...
  void shear(NumMatrix<Complex>& polarCoeffs, data_t gamma0, data_t gamma1, std::ostringstream& history);
  ...
  void flex(NumMatrix<data_t>& cartesianCoeffs, const NumMatrix<data_t>& dGamma, std::ostringstream& history);
  /// Translate the image by \f$dx0, dx1\f$.
  /// This is only valid for small translations (below 1 pixel).
  void translate(NumMatrix<data_t>& cartesianCoeffs, data_t beta, data_t dx0, data_t dx1, std::ostringstream& history);
  ...
  void brighten(NumMatrix<data_t>& cartesianCoeffs, NumMatrix<Complex>& polarCoeffs, data_t factor, std::ostringstream& history);
  void convolve(NumMatrix<data_t>& cartesianCoeffs, data_t& beta, const NumMatrix<data_t>& KernelCoeffs, data_t beta_kernel, std::ostringstream& history);  
  void deconvolve(NumMatrix<data_t>& cartesianCoeffs, data_t& beta, const NumMatrix<data_t>& KernelCoeffs, data_t beta_kernel, std::ostringstream& history);
  /// Change the scale size of the image by using rescaling relation.
  /// see Paper I, appendix A.
  void rescale(NumMatrix<data_t>& cartesianCoeffs, data_t beta, data_t newbeta, std::ostringstream& history);
